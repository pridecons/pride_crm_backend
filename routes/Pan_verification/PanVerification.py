...
@router.get("/pan-cache/{pannumber}")
def get_pan_cache(
    pannumber: str,
    db: Session = Depends(get_db)
):
    """
    Get cached PAN verification data
    """
    try:
        pannumber = pannumber.upper().strip()
        ...
        if not entry:
            raise HTTPException(
                status_code=404,
                detail="No cached data found for this PAN"
            )
        
        if not entry.response:
            raise HTTPException(
                status_code=404,
                detail="No verification data available for this PAN"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail="Cached data is corrupted"
        )
    except HTTPException:
        raise
    except Exception as e:
...
@router.delete("/pan-cache/{pannumber}")
def delete_pan_cache(
    pannumber: str,
    db: Session = Depends(get_db)
):
    """
    Delete cached PAN verification data
    """
    try:
        pannumber = pannumber.upper().strip()
        ...
        return {
            "success": True,
            "message": f"Cached data for PAN {pannumber} deleted successfully"
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error deleting cached data: {str(e)}"
        )
