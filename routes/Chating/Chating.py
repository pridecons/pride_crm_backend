...
def create_direct(
    ...
    branch_id = me.branch_id if _ensure_same_branch(me.branch_id, peer.branch_id) else (peer.branch_id if me_role == "SUPERADMIN" else me.branch_id)
...
@router.post("/{thread_id}/send", response_model=MessageOut, status_code=201)
async def send_message(  # <-- async to await broadcast
    thread_id: int,
    payload: SendMessageIn,
    db: Session = Depends(get_db),
    me: UserDetails = Depends(get_current_user),
    ...
    # ---- WS broadcast so frontend doesn't need to post to WS ----
    # Use a stable event shape for clients
    ...
    await room_hub.broadcast_json(
        thread_id,
        {
            "type": "message.new",
            "thread_id": thread_id,
            "message": out,  # created_at becomes ISO via model_dump(mode="json")
        },
    )
    # -------------------------------------------------------------
...
@router.post("/{thread_id}/mark-read", status_code=204)
async def mark_read(  # async to broadcast receipt
    thread_id: int,
    last_message_id: int,
    db: Session = Depends(get_db),
    me: UserDetails = Depends(get_current_user),
    ...
    # Optional: push a read-receipt event so UIs can update ticks/badges
    await room_hub.broadcast_json(
        thread_id,
        {
            "type": "message.read",
            "thread_id": thread_id,
            "reader_id": me.employee_code,
